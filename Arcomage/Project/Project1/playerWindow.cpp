...
PlayerWindow::PlayerWindow(PlayerState &_playerState, const Settings &_settings) :
	settings(_settings),
...
const wchar_t* PlayerWindow::nameClassWindow = L"ClassPlayerWindow";
const wchar_t* PlayerWindow::nameWindow = L"PlayerWindow";
...
HWND PlayerWindow::GetHandle() const {
	return handle;
}
...
void PlayerWindow::drawBuildings(HDC dc) const {
	RECT clientRect;
...
void PlayerWindow::drawStats(HDC dc) const {
	RECT clientRect;
	::GetClientRect(handle, &clientRect);
